...
package body Periodic_Tasks is
   ...
   begin
      ...
      --- 1.3
      ---Info_L := Periodic_Task_Table(L);
      ---Info_R := Periodic_Task_Table(R);
      ---Put_Natural (L);
      ---Put_Natural (R);
      return Periodic_Task_Table (R).Period < Periodic_Task_Table (L).Period;
      ---if Info_L.Period < Info_R.Period then
      ---return False;
      ---else
      ---return True;
      ----end if;
   ...
   begin
	 ...
	 Deadline := Deadline + Info.Period;  --- 1.3
